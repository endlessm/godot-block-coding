...
func _ready():
	...
	%Background.custom_minimum_size.x = Constants.CONTROL_MARGIN
	...
	%SnapContainer.add_theme_constant_override("margin_left", Constants.CONTROL_MARGIN)
...
func format():
	arg_name_to_param_input_dict = StatementBlock.format_string(self, %RowHBox, block_resource.block_format, block_resource.defaults)
